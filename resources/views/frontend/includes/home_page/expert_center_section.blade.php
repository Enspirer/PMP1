<section id="expert-center">
    <div class="title" style="max-width: 400px">
        <h1 data-animscroll="fade-up" style="font-size: 42px;color: #361d47">Expert Center</h1>
        <hr data-animscroll="fade-up"/>
    </div>
    <div class="container">
        <div class="row">
            <div class="left-text col-md-6">
                <h3 data-animscroll="fade-up">Plan Your Project with <br> our experts</h3>
                <p style="font-size: 22px;color: #f44738;font-weight: 700">Plan it precisely, Save money and time</p>
{{--                <ul>--}}
{{--                    <li data-animscroll="fade-up">--}}
{{--                    </li>--}}
{{--                    <li data-animscroll="fade-up">--}}
                        The biggest challenge for any company is to define the scale and the specifications of a projects.
                <hr>
{{--                    </li>--}}
{{--                    <li data-animscroll="fade-up">--}}
                        Our professional team will take care of the end to end planning of your project to take the load off your mind.
                <hr>
{{--                    </li>--}}
{{--                    <li data-animscroll="fade-up">--}}
                <span style="font-weight: bold">Step 01</span> – Register as a company and request for a consultation
                <hr>
{{--                    </li>--}}
{{--                    <li data-animscroll="fade-up">--}}
                <span style="font-weight: bold">Step 02</span> – One of our members will arrange time for virtual meeting.
                <hr>
{{--                    </li>--}}
{{--                    <li data-animscroll="fade-up">--}}
                <span style="font-weight: bold">Step 03</span> – Brief your requirements
                <hr>
{{--                    </li>--}}
{{--                </ul>--}}
                <br>
                <a href="{{route("frontend.expert_center")}}" data-animscroll="fade-up" style="margin: 0;background: #f44738;border: none;color: #fff;padding: 12px 25px">Book your 15 Min FREE consultation</a>
            </div>
            <div class="right-image col-md-6">
                <img src="{{url('theme_light/assets/image/expert.png')}}" alt="" data-animscroll="fade-left">
            </div>
        </div>
    </div>
</section>
